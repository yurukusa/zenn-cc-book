...
この記事では、PostToolUseフックを使った7つの実践パターンを紹介する。すべて`npx cc-safe-setup`で導入できるhook群（446個・6,099テストで検証済み）から抜粋したものだ。
...
これだけで、446個のhookが一括で導入される。上記の7パターンはその一部だ。すべて6,099のテストで動作が検証されている。
