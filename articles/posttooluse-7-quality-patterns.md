...
この記事では、PostToolUseフックを使った7つの実践パターンを紹介する。すべて`npx cc-safe-setup`で導入できるhook群（443個・6,000テストで検証済み）から抜粋したものだ。
...
これだけで、443個のhookが一括で導入される。上記の7パターンはその一部だ。すべて6,000のテストで動作が検証されている。
