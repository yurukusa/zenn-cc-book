...
この記事では、PostToolUseフックを使った7つの実践パターンを紹介する。すべて`npx cc-safe-setup`で導入できるhook群（421個・5,668テストで検証済み）から抜粋したものだ。
...
これだけで、421個のhookが一括で導入される。上記の7パターンはその一部だ。すべて5,668のテストで動作が検証されている。
