...
この記事では、PostToolUseフックを使った7つの実践パターンを紹介する。すべて`npx cc-safe-setup`で導入できるhook群（425個・5,695テストで検証済み）から抜粋したものだ。
...
これだけで、425個のhookが一括で導入される。上記の7パターンはその一部だ。すべて5,695のテストで動作が検証されている。
