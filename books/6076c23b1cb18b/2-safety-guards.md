...
8つの基本フックに加えて、プロジェクトに特化したガードが必要なことがある。cc-safe-setupには667個のexample hookが同梱されている:
...
**次章以降で解決する痛み:**
- **第3章**: 「30ファイルに構文エラーが伝播した」→ 編集直後に自動検出してブロック
- **第4章**: 「トークン消費が突然10倍になった」→ キャッシュ汚染の検知と防止
- **第5章**: 「3時間のセッションが後半ボロボロになった」→ コンテキスト劣化の検知と自動リカバリ
- **第9章**: 「どのhookを入れればいいか分からない」→ 全hookカタログ（カテゴリ別・解説付き）
