---
title: "全工程チェックリスト——19点→85点の完全手順"
free: false
---

# 全工程チェックリスト——19点→85点の完全手順
...
これだけで8個のコアhook + プロジェクト検出 + 推奨hookが全てインストールされる。658個のhookから最適な組み合わせを自動選択する。
...
# 追加のexample hookをインストール（658種から選択可能）
...
筆者の環境では19点→85点だった（一部チェックは運用上スキップしたため）。全チェックを実施した場合の理論上の変化:
...
ルールをCLAUDE.mdに書いても守られないことがある。GitHubでは[1,000件以上のリアクション](https://github.com/anthropics/claude-code/issues/42796)が付いたIssueで「指示が無視される」問題が報告されている。これは設定ミスではなく、LLMの構造的な限界だ。でもhookでブロックすれば物理的に止まる。監視ツールを入れれば問題を知ることができる。ログがあれば原因を追える。
...
**658個のexample hookからプロジェクトに合ったものを追加**:
