...
`branch-guard`はデフォルトの8 hookに含まれている（`npx cc-safe-setup`で自動インストール）。mainへのpushとforce-pushをブロック。`work-hours-guard`は業務時間外のリスク操作を制限する追加hookだ。
...
**何が起きたか：** 複雑なタスクを実行中、突然「Claude's response exceeded the 32000 output token maximum」エラーが出てセッションが中断。GitHub Issue [#24055](https://github.com/anthropics/claude-code/issues/24055)（82リアクション）で多数報告。
...
**何が起きたか：** Max Planの5時間制限が1時間で枯渇。以前と同じ使い方なのに、トークン消費速度が数倍に感じる。GitHub Issue [#40524](https://github.com/anthropics/claude-code/issues/40524)（220リアクション）を筆頭に、2026年3月末から大量の報告が集まっている。
...
- **セッション再開（`--resume`）**: 再開時にシステムリマインダーブロック（skills listing等、約3,800トークン相当）が`messages[0]`から消え、会話履歴の深い位置に移動する。これによりキャッシュプレフィックスが変わり、**毎ターン約3,800トークンのキャッシュミス**が発生する（[#44045](https://github.com/anthropics/claude-code/issues/44045)で確認）。対策: 長時間のマルチターンセッションでは`--resume`より新規セッション開始を検討する
...
**見えない消費: ツール結果の自動切り詰め（2026年4月時点で未修正）：**

上記のキャッシュ破壊とは別に、Claude Code内部で**ツール結果が自動的に短縮される**仕組みがある。セッション中にツール呼び出しを繰り返すと、古い結果が1〜41文字まで切り詰められる。
...
- 1セッションで261回の切り詰めイベントが計測された実例あり（[ArkNill氏のプロキシ分析](https://github.com/ArkNill/claude-code-hidden-problem-analysis)、[#42542](https://github.com/anthropics/claude-code/issues/42542)）

結果として、Claudeは以前読んだファイルの内容を「覚えていない」状態になり、同じファイルを再度Readする。この再読み込みがトークンを消費する悪循環になる。

**対策:** 長いセッションではこまめに`/compact`し、重要な情報はCLAUDE.mdやタスクファイルに外部化する。ツール結果に頼らず、必要な情報を永続的な場所に書き出す習慣が有効。

**見えない消費: 偽レート制限（2026年4月時点で未修正）：**

セッションログに`"model":"<synthetic>"`というエントリが混入することがある。これはAPIを実際には呼び出していないのに、レート制限のカウントに計上されるエントリだ。

- ArkNill氏の65セッション分析で、151個の`<synthetic>`エントリが発見された
- `input_tokens:0`——つまりトークンを消費していないのに、使用率カウントが増える
- 症状は「まだ全然使っていないのにUsage limit reachedが出る」
- 並行サブエージェントの実行が原因の可能性が高い（[#40584](https://github.com/anthropics/claude-code/issues/40584)）
...
**対策:** 現時点で完全な回避策はない。サブエージェントの並行実行数を抑えることで発生頻度を下げられる可能性がある。

**見えない消費: ログのトークン水増し（2026年4月時点で未修正）：**

`/cost`コマンドで表示されるトークン消費量が、実際の消費量より大きく表示される場合がある。Extended ThinkingのトークンがJSONLログに重複記録されるためだ。

- サブエージェント: 1.71〜2.67倍の膨張
- メインエージェント: 最大2.87倍の膨張
- サーバー側の実課金には影響しない（ローカルログの問題）
- v2.1.92で`/cost`にキャッシュヒット内訳が追加されたが、水増し自体は修正されていない（[#41346](https://github.com/anthropics/claude-code/issues/41346)）
...
# /costの表示と比較。2倍以上の乖離があれば水増しの可能性
```

**対策:** `/cost`の表示値を鵜呑みにしない。「思ったより消費が激しい」と感じたら、表示の約半分が実際の消費量と見積もる。
...
:::message
**トークン消費を体系的に削減したい方へ**: このパターン10で紹介した対策は入り口に過ぎない。CLAUDE.mdの構造最適化、コンテキスト管理、ワークフロー設計まで含む全体最適化と800時間の実測データは[Token Book](https://zenn.dev/yurukusa/books/token-savings-guide)（¥2,500）に収録。まずは無料の[Token Checkup](https://yurukusa.github.io/cc-safe-setup/token-checkup.html)で診断。
:::
