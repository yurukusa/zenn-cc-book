---
title: "hookのテスト——13,931テストケースから学んだ品質保証"
---
...
cc-safe-setupには629個のexample hookがある。初期は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で13,931テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 9201/9201 passed
```
...
1. **テストされていないhookは信頼できない**。629個のhookをテストなしで配布していた期間は、品質を保証できなかった
