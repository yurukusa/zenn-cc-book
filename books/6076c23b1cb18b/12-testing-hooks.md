...
cc-safe-setupには658個のexample hookがある。初期（当時511個）は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
1. **テストされていないhookは信頼できない**。658個のhookをテストなしで配布していた期間は、品質を保証できなかった
