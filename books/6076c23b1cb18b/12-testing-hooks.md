---
title: "テストのないhookが本番で偽陽性を出した——14,000超のテストを書くことになった経緯"
---
...
cc-safe-setupには650個以上のexample hookがある。初期（当時511個）は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で全テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 14000+/14000+ passed（テスト数は増加中）
```
...
1. **テストされていないhookは信頼できない**。650個以上のhookをテストなしで配布していた期間は、品質を保証できなかった
