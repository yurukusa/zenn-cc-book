---
title: "hookのテスト——7,789テストケースから学んだ品質保証"
---
...
cc-safe-setupには539個のexample hookがある。初期は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で7,789テストを一括実行する:
...
1. **テストされていないhookは信頼できない**。539個のhookをテストなしで配布していた期間は、品質を保証できなかった
