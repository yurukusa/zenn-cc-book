---
title: "hookのテスト——7,341テストケースから学んだ品質保証"
---
...
cc-safe-setupには507個のexample hookがある。初期は118個しかテストがなかった。残り337個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で7,341テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 7341/7341 passed
```
...
1. **テストされていないhookは信頼できない**。507個のhookをテストなしで配布していた期間は、品質を保証できなかった
