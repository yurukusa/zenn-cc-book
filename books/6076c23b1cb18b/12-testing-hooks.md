---
title: "hookのテスト——7,564テストケースから学んだ品質保証"
---
...
cc-safe-setupには511個のexample hookがある。初期は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で7,564テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 7349/7349 passed
```
...
1. **テストされていないhookは信頼できない**。511個のhookをテストなしで配布していた期間は、品質を保証できなかった
