---
title: "hookのテスト——1,712テストケースから学んだ品質保証"
---
...
cc-safe-setupには332個のexample hookがある。初期は118個しかテストがなかった。残り213個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で1,712テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 1712/1712 passed
```
...
1. **テストされていないhookは信頼できない**。332個のhookをテストなしで配布していた期間は、品質を保証できなかった
