---
title: "hookのテスト——9,201テストケースから学んだ品質保証"
---
...
cc-safe-setupには605個のexample hookがある。初期は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で9,201テストを一括実行する:
...
1. **テストされていないhookは信頼できない**。605個のhookをテストなしで配布していた期間は、品質を保証できなかった
