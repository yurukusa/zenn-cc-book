---
title: "hookのテスト——6,867テストケースから学んだ品質保証"
---
...
cc-safe-setupには500個のexample hookがある。初期は118個しかテストがなかった。残り330個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で6,867テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 6867/6867 passed
```
...
1. **テストされていないhookは信頼できない**。500個のhookをテストなしで配布していた期間は、品質を保証できなかった
