---
title: "hookのテスト——4,572テストケースから学んだ品質保証"
---
...
cc-safe-setupには358個のexample hookがある。初期は118個しかテストがなかった。残り240個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で4,572テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 4572/4572 passed
```
...
1. **テストされていないhookは信頼できない**。358個のhookをテストなしで配布していた期間は、品質を保証できなかった
