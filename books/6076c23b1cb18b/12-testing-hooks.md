---
title: "テストのないhookが本番で偽陽性を出した——14,078テストを書くことになった経緯"
---
...
cc-safe-setupには653個のexample hookがある。初期（当時511個）は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で14,078テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 14078/14078 passed
```
...
1. **テストされていないhookは信頼できない**。653個のhookをテストなしで配布していた期間は、品質を保証できなかった
