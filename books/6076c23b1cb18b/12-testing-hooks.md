---
title: "テストのないhookが本番で偽陽性を出した——9,200超のテストを書くことになった経緯"
---
...
cc-safe-setupには667個以上のexample hookがある。初期（当時511個）は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
bash test.sh
# Results: 9200+/9200+ passed（テスト数は増加中）
```
...
1. **テストされていないhookは信頼できない**。667個以上のhookをテストなしで配布していた期間は、品質を保証できなかった
