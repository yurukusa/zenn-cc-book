---
title: "hookのテスト——8,984テストケースから学んだ品質保証"
---
...
cc-safe-setupには592個のexample hookがある。初期は118個しかテストがなかった。残り393個は「たぶん動く」状態だった。
...
cc-safe-setupでは `bash test.sh` で8,984テストを一括実行する:

```bash
cd /path/to/cc-safe-setup
bash test.sh
# Results: 8984/8984 passed
```
...
1. **テストされていないhookは信頼できない**。592個のhookをテストなしで配布していた期間は、品質を保証できなかった
