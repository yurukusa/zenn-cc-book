...
:::message
**トークン消費をさらに深掘りしたい方へ**: この章ではモニタリングhookを紹介したが、CLAUDE.mdの書き方、コンテキスト管理、ワークフロー設計まで含めた体系的な最適化手法は[Token Book — Claude Codeのトークン消費を半分にする](https://ko-fi.com/s/fd44ef09a7)（¥2,500）にまとめた。800時間の実測データと設定テンプレート付き。まずは無料の[Token Checkup](https://yurukusa.github.io/cc-safe-setup/token-checkup.html)で自分の消費パターンを診断できる。
:::
