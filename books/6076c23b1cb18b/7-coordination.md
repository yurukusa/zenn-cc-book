...
**入手方法**: `npx cc-safe-setup --install-example decision-warn`（[cc-safe-setup](https://github.com/yurukusa/cc-safe-setup)）
...
このルール（セッション開始時にlessons.mdを読む）はCLAUDE.md自律運用テンプレートに含まれている（[cc-safe-setup](https://github.com/yurukusa/cc-safe-setup)参照）。

## チェック21: サブエージェントの品質を検証する

**失敗例**: 調査タスクをサブエージェントに委任した。サブエージェントは「調査完了。auth flowはJWTベースで安全です」と報告。実際にファイルを読んでいなかった。メインのCCはこの報告をそのまま伝えた。

**根本原因**: サブエージェントが曖昧な指示で起動され、結果も検証されていなかった。

**対策**: 2つのhookで委任品質と結果品質の両方をチェックする。

### 委任時の品質チェック（PreToolUse）

```bash
npx cc-safe-setup --install-example subagent-scope-validator
```

サブエージェント起動時に委任プロンプトを検査する:
- プロンプトが100文字未満 → 「曖昧すぎる」と警告
- ファイルパスの指定なし → 「何を読むべきか指定しろ」と警告
- 具体的な動詞がない → 「何をすべきか明記しろ」と警告

曖昧な委任をブロックはしない（advisory）が、警告されるだけで品質が大幅に上がる。

### 結果の品質チェック（PostToolUse）

```bash
npx cc-safe-setup --install-example subagent-error-detector
```

サブエージェントが返した結果を検査する:
- 529 Overloaded / 502 Bad Gateway → 「APIエラーで失敗している」と警告
- タイムアウト → 「結果が返っていない」と警告
- 50文字未満の結果 → 「結果が薄すぎる」と警告

並列サブエージェントが全滅しても、メインのCCが気づかずに続行する問題を防ぐ。
...
```
  ▸ Coordination
    ...
    [PASS] Subagent delegation and results verified
    100%
```
