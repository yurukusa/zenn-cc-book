...
この3つのコマンドで、スコアは一桁から42点前後まで上がる。

だが42点と85点の差は**「見える危険」と「見えない危険」の差**だ。

ファイル削除やmainへのpushは目に見える。hookでブロックすれば済む。700時間の運用で一番痛かったのは見えない方だった——キャッシュ読取率が89%→4%に静かに暴落してトークンが20倍消える、コンテキストが劣化して3時間の作業を最初からやり直す。これらはhookの有無ではなく、**監視と運用の設計**で防ぐ。第4章以降で扱う。
