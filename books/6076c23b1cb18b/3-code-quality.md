...
GitHub Issue [#40524](https://github.com/anthropics/claude-code/issues/40524)（195👍）で「Max Planの5時間分が15分で消えた」という報告が相次いでいる。原因の多くはプロンプトキャッシュの無効化で、キャッシュ読み取り率が89%→4%に暴落すると消費量が10〜20倍になる。第4章で、この暴走を検知するhookとトークン消費を可視化するロガーの実装を解説する。
