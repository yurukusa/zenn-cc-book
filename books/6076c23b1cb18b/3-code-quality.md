...
だが85点に到達するには、あと4つの領域が要る。これらは`npx cc-safe-setup`には含まれていない——hookの設定ではなく、環境に合わせた監視と運用の設計が必要になる。
...
第10章には、700時間の運用で実際に遭遇した「やらかし」10パターンをまとめた。たとえば:

> 自律モードで放置していたClaude Codeが深夜3時にmainブランチにforce-push。朝来たらCIが全壊していた。——パターン1
