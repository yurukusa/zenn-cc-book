---
title: "高度なhookパターン——Claudeがhookをバイパスした日"
---
...
**空文字列matcherの罠**: matcher `""` は全ツールにマッチする。hookに構文エラーがあると、`exit 2`が返り、**全ツール**がブロックされる。3時間のロックアウトを経験した。
...
:::note info
**トークン消費を半分にする方法**
hookで安全を守れたら、次はトークン効率だ。CLAUDE.mdの書き方だけで消費が2倍変わる。800時間の実測データと設定テンプレート付き:
- [Token Book — Claude Codeのトークン消費を半分にする](https://zenn.dev/yurukusa/books/token-savings-guide)（¥2,500）
- まずは無料の[Token Checkup](https://yurukusa.github.io/cc-safe-setup/token-checkup.html)で診断
:::
...
- [Claude Code Skills 実践レシピ集](https://zenn.dev/yurukusa/books/e0c5dfc1b7b157)（¥500）
:::

:::note info
**この本の著者がClaude Codeを800時間走らせた全記録**
非エンジニアが$600を投じて何が起きたか——事故、収益の現実、再現可能な8つのフレームワーク。失敗も数字もそのまま書いた体験記です:
- [AIに仕事を任せてみた——非エンジニアが800時間で学んだ全記録](https://zenn.dev/yurukusa/books/3c3c3baee85f0a19)（¥800・第2章まで無料）
:::
