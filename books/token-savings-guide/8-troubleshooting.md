...
## 症状56: v2.1.118 `/usage` が固定料金プランなのに "API 課金額" を表示する（2026-04-23〜）

### 症状

- v2.1.118 で `/cost` が `/usage` に統合された直後から、**定額制の Max/Pro 契約者にも "API pay-per-usage" のドル金額が表示される**
- 表示される金額は自分が実際に払う請求額ではなく、「もしこのセッションが API 従量課金だったらいくらか」の参考値
- 具体的には `Usage Cost: $X.XX` のような行が出る（[#52365](https://github.com/anthropics/claude-code/issues/52365), v2.1.118 darwin, 3 reactions, OPEN）
- 報告者コメント: "pretty scary" — 固定料金で契約したのに追加請求されているかのような不安を起こす

### 原因

`/cost`（開発者向けのトークン消費表示）と `/usage`（ユーザー向けのクォータ残量表示）が v2.1.118 で統合された際、**従量課金相当のドル換算ロジックが固定料金契約にも発動してしまう**。Max/Pro は月額一括なので、この表示は契約とは無関係だが、UI 側で「プラン種別による条件分岐」が抜けている。

### なぜトークン対策として重要か

固定料金 Max/Pro ユーザーがこの表示を真に受けると、2種類の誤った判断を招く:

1. **「このまま使うと追加請求される」と誤解して作業を止める**: 実際には定額の範囲内であり、止める必要はない。しかし画面に `$` が出ていると、心理的に「使い過ぎ＝追加課金」の連想が働く。本書で解説してきた「`/cost` で実消費を監視する」習慣が、**かえってユーザーを萎縮させる副作用**を生む
2. **逆に「¥0 と出ているから使っていい」と過信する**: 一部プランでは表示が `$0.00` に張り付くことがあり、実際にはクォータを消費しているのに "無料" の錯覚を起こす

本書では症状 10（v2.1.111+ システムトークン膨張）以降で繰り返し「`/cost`/`/usage` で実測する」ことを推奨してきた。v2.1.118 以降は、**「ドル金額の数字」ではなく「token 消費量と cache_read 比率」だけを見る**運用に切り替える必要がある。

### 対処法

1. **`/usage` のドル金額行は無視する**: 自分のプランが Max/Pro の固定料金なら、表示された `$X.XX` は実際の請求と無関係。追加課金は発生していない
2. **token 数と cache_read 比率だけ読む**: `Input tokens`、`Output tokens`、`Cache read tokens` の3行だけを監視する運用にする。ドル額は **Anthropic がプラン種別判定バグを直すまで表示ノイズ**と割り切る
3. **API 従量課金ユーザーは逆に警戒を強める**: `/usage` のドル額は自分に請求される金額と一致するので、このバグの影響は受けない。むしろ固定料金向けの表示ミスを意識して、**自分の数字は必ず Anthropic Console の Billing ページでクロスチェック**する
4. **不安になったら `/usage --json` で生データを取る**: `claude /usage --json` は請求にかかる実数だけ返す（UI レイヤーのドル換算を経由しない）。`"input_tokens": N, "cache_read_input_tokens": M` のような raw 値だけを信頼する

### 既存購入者への申し送り

本書第1章・第4章・第7章・本章の各所で「`/cost` で消費を監視する」「UI の %表示より `/cost` を信頼する」と書いてきたが、v2.1.118 以降の `/usage` が示すドル額については **プラン種別によって解釈を変える必要がある**。Max/Pro 固定料金ユーザーは、ドル額を見ずに token 数だけを見る運用に移行してほしい。この変更は Anthropic が #52365 を fix すれば元に戻せる。

### 本章追加5症状（52〜56）の共通教訓

- 症状52/53/54は[Issue #46829](https://github.com/anthropics/claude-code/issues/46829)の**cache 側**のユーザー発見パターン、症状55は**quota メーター側**の同日クラスタ、症状56は**v2.1.118 の UI 統合による表示バグ**
- どれもAnthropic公式postmortemや changelog には含まれない**運用側で先に気付く必要がある痛み**
- 対処法はすべてユーザー側でできる。ただし症状55・56は Anthropic 側のサーバー/クライアント bug 確定のため、**ログで証拠を残してサポートに上げる** ことも長期的対処になる
