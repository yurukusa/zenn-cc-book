...
7. **task_budgetで消費上限を設定する（API利用者向け）**: Opus 4.7で追加された `task_budget` を使えば、1回のエージェントループ全体（thinking / ツール呼び出し / 出力）にトークン予算を設定できる。`max_tokens` が1リクエストのハード上限なのに対し、`task_budget` はモデルが認識する目標値で、予算が減るとモデルが自発的に作業をまとめて終了する。長いデバッグセッションで予算超過を防ぐのに有効
...
:::message
新モデルが必ずしもコスト効率が良いとは限らない。「最新=最良」ではなく、自分のユースケースで検証してから切り替えること。`task_budget` は 2026-04 時点 public beta。
:::
...
1. **`/usage`で頻繁にチェック**: cache_readの比率が異常に低い場合、この問題に該当する可能性がある
...
ここまでの症状の多くは、4つのhookで未然に防げる:

- `large-read-guard` — 大きなファイルの丸読みを警告（PreToolUse / Read）
- `subagent-budget-guard` — サブエージェントの乱用を制限（PreToolUse / Agent）
- `token-budget-guard` — トークン予算の超過を警告（PostToolUse）
- `pre-compact-checkpoint` — /compact前に作業を自動保存（PreCompact）

設定例とインストール手順は[cc-safe-setupのexamples](https://github.com/yurukusa/cc-safe-setup/tree/main/examples)にある。`npx cc-safe-setup --install-example <name>` で個別導入できる。
...
**deny rulesではなく、PreToolUse hookで防御する**。hookはサブコマンド数の制限を受けない。サブコマンドを `;`/`&`/`|` で分割してカウントし、閾値超過で exit 2 で止める。実装は cc-safe-setup の [subcommand-chain-guard](https://github.com/yurukusa/cc-safe-setup/blob/main/examples/subcommand-chain-guard.sh) を使うか、`npx cc-safe-setup --install-example subcommand-chain-guard` でインストール。
...
1. **`/usage`で毎ターン確認する習慣をつける**: 異常な増加に気づいたらセッションを終了する
...
1. **`/usage`で実際の消費を監視する**: UIの表示ではなく`/usage`コマンドの数字を信用する
...
1. **UIの%表示を信用しない**: `/usage`コマンドで実際のトークン数を確認する
...
1. **PreToolUseフックでブロック**: Bashコマンド実行前に`claude`/`npx claude`を正規表現検出して `exit 2` で止める。実装は cc-safe-setup の [recursive-spawn-guard](https://github.com/yurukusa/cc-safe-setup/blob/main/examples/) を参照
...
3. **`/usage`で消費を確認する**: スタック後は必ず`/usage`でどれだけquotaが消費されたか確認する
...
1. **`dotenv-read-guard` hookをインストールする**: PreToolUse hookはサブエージェントにも継承される。hookで`.env`ファイルの読み取りをブロックすれば、メインエージェント・サブエージェント両方で防御できる。実装は cc-safe-setup の [dotenv-read-guard](https://github.com/yurukusa/cc-safe-setup/blob/main/examples/dotenv-read-guard.sh) を参照
...
1. **`exploration-budget-guard` hookをインストールする**: Read/Glob/Grep を連続カウントし、25回で警告・40回でブロック。Edit/Write が走るとリセット。実装は cc-safe-setup の [exploration-budget-guard.sh](https://github.com/yurukusa/cc-safe-setup/blob/main/examples/exploration-budget-guard.sh) を参照
...
4. **夜間放置前に`/usage`を記録する**: 翌朝の比較で異常消費を検出
...
Extended Thinkingには**思考トークンの上限が設定されていない**。モデルが解決策を見つけられないまま推論ループに入ると、コンテキストウィンドウの限界まで思考トークンを生成し続ける。`/usage`で確認しない限り、外部からは進行中に見える。30分以上のフリーズ（[#49884](https://github.com/anthropics/claude-code/issues/49884)）と同じパターンだが、トークン消費の規模が桁違いに大きい。

### 対処法

1. **`thinking-stall-detector.sh` hookを設置する**: 思考時間が閾値（5分等）を超えたらアラート
2. **セッションを無人放置しない**: 特にExtended Thinking有効時
3. **`/usage`を定期的に確認する**: 出力がないのにトークンが急増していたら即座にセッションを再起動
...
effort=85は「中程度の思考量」を意味するが、実際には多くのタスクに対して過剰。簡単な質問やファイル読み取りにも85%の思考リソースが投入される。ユーザーは明示的にeffort値を設定していないため、デフォルトが高すぎることに気づかない。`/usage`コマンドの出力にはeffort値が表示されないため、原因特定が困難。
...
1. **effort値を明示的に下げる**: `/usage`でトークン消費を確認後、簡単なタスクには`--effort low`（effortスキーマで指定）を使う
...
4. **`/usage`を定期的に確認する**: セッション開始時と30分後に`/usage`を実行し、消費ペースが予想を超えていないか確認
...
2. **`/usage`で定期的に確認**: 想定より回答が短い・浅いと感じたら、コスト表示でeffort低下を疑う
...
つまり、Telemetryを切ると**experiment gateが無効化され、cache TTLのデフォルト値である5分が強制適用される**。1時間cache TTLの最適化はexperimentとして配布されているため、Telemetry OFFのユーザーは（experiment gate 経由では）恩恵を受けられない。

**v2.1.108（2026-04-14）以降の追加 path**: Anthropic は同 Issue で約束した env var を ship 済 ── `ENABLE_PROMPT_CACHING_1H=1` を export すると、telemetry の on/off に関わらず 1h TTL を強制できる。Telemetry OFF を維持したまま 1h cache の恩恵を取りに行けるようになった ([changelog v2.1.108](https://code.claude.com/docs/en/changelog#2-1-108))。なお v2.1.121 には Issue [#54485](https://github.com/anthropics/claude-code/issues/54485) で報告された `cache_control` payload reject バグがあり、v2.1.121 を使っているユーザーはこの env var を有効化する前に修正版を待つことを推奨。
...
1. **`ENABLE_PROMPT_CACHING_1H=1` を export する（最も直接的）**: v2.1.108 以降であれば、Telemetry の on/off と無関係に 1h TTL を強制できる。`~/.bashrc` か Claude Code 起動 wrapper script で設定。v2.1.121 を使っている場合は Issue #54485 修正版を待つ
2. **Telemetryを切る前にトレードオフを理解する**: プライバシーvsキャッシュ最適化。env var が ship されたので「telemetry を on にしないと cache 最適化が効かない」 という旧前提は崩れた。プライバシー優先で OFF + env var ON が新しい既定組み合わせ
3. **どうしても env var も使いたくない場合**: `/usage`で定期的にcache_read比率を監視。50%を大幅に下回る状態が続いていたら、env var 設定を検討（Telemetry 再 ON より低 friction）
4. **企業環境でPolicyで強制OFFされている場合**: IT担当と交渉する材料として本症状を共有する（社員の月$20〜50の無駄課金を定量化できる）。env var の export は通常 IT policy の OFF 強制とは独立して効くため、env var 経由で最適化できる可能性が高い
