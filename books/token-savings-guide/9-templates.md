...
cc-safe-setupには664以上のhookが用意されている。本書で紹介した9個のhookのうち、`large-read-guard`、`read-budget-guard`、`token-budget-guard`、`token-spike-alert`、`subagent-budget-guard`、`pre-compact-checkpoint`、`session-budget-alert`の7つはcc-safe-setupに含まれている。`duplicate-read-detector`と`bash-output-guard`は本書オリジナルのため、上記テンプレート3からコピーして使用する。
