...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Read",
        "command": "bash ~/.claude/hooks/large-read-guard.sh",
        "timeout": 5000
      },
      {
        "matcher": "Read",
      ...
      {
        "matcher": "Agent",
        "command": "bash ~/.claude/hooks/subagent-budget-guard.sh",
        "timeout": 5000
      },
      {
        "matcher": "Bash",
    ...
    "PostToolUse": [
      {
        "matcher": "",
        "command": "bash ~/.claude/hooks/token-budget-guard.sh",
        "timeout": 5000
      },
      {
        "matcher": "",
    ...
    "PostCompact": [
      {
        "matcher": "",
        "command": "bash ~/.claude/hooks/post-compact-safety.sh",
        "timeout": 5000
      }
    ],
    "SessionStart": [
      {
        "matcher": "",
        "command": "bash ~/.claude/hooks/session-budget-alert.sh",
        "timeout": 5000
      }
    ]
  }
...
**ポイント**: 第4章で紹介した10個のhookのうち、最も効果が高い9個を設定。`timeout`は各hook 5秒（checkpoint のみ10秒）。
...
### post-compact-safety.sh

```bash
#!/bin/bash
# 圧縮後の暴走防止
echo "📋 コンテキスト圧縮完了。次の指示を待ってください。"
...
cc-safe-setupには600以上のhookが用意されている。本書で紹介した10個以外にも、セキュリティ、Git保護、コード品質など幅広いカテゴリがある。
...
export CC_READ_WARN=120        # デフォルト: 80
