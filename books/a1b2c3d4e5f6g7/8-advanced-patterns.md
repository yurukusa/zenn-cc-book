...
スキル間の連携は、SKILL.md本文の手順に別のスキルのトリガーワードを含めることで実現できる場合がある。Claude Codeがその手順を実行するとき、トリガーワードに反応して対応するスキルが読み込まれる。ただし、これは常に保証される動作ではなく、スキルの認識はセッション状態やコンテキスト量に依存する。確実に連携させたい場合は、手順中で明示的に「/skill-nameを実行して」と書く方が安定する。
...
2. スキル間連携はトリガーワードまたは明示的な指示で実現する
...
:::message
**トークン消費を半分にする実践ガイド**
Progressive Disclosureで節約したその先、さらに半分に減らすための800+時間の実測データと4月の公式インシデント（Tokenocalypse）の原因分析。extractMemoriesの1行OFF設定だけでAPIコストが最大50%下がる理由も解説:
